...
class ThreadPool {
   ...
   public:
    ...
    bool IsRunning() const { return m_ActiveThreads != 0; };
    ...
    void AddTask(Function&& func, Args&&... args) {
        ...
        m_Tasks.emplace([bound_func]() -> void {
            bound_func();
        });
    ...
    void AddTaskQE(Function&& func, Args&&... args) {
        ...
        auto bound_func = std::bind(std::forward<Function>(func), std::forward<Args>(args)..., std::ref(m_QuickExitSignal));
        m_Tasks.emplace([bound_func]() -> void {
            bound_func();
        });
        m_NotifyTask.notify_one();
